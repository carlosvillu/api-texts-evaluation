...
- [ ] **1.1.1** Crear directorio principal `text-evaluation-service/`
- [ ] **1.1.2** Crear subdirectorios:
  - `app/` (código principal)
  - `app/routes/` (endpoints)
  - `app/services/` (lógica de negocio)
  - `app/utils/` (utilidades)
  - `scripts/` (scripts auxiliares)
- [ ] **1.1.3** Crear archivos `__init__.py` necesarios
...
- [ ] **1.2.1** Crear `requirements.txt` con dependencias:
  ```txt
  fastapi>=0.104.0
...
- [ ] **1.2.2** Crear `.env.example` con variables de entorno:
  ```env
  # Model Configuration
...
- [ ] **1.2.3** Crear `Dockerfile` optimizado para GPU L40:
  ```dockerfile
...
- [ ] **1.2.4** Crear `docker-compose.yml` para desarrollo:
  ```yaml
  version: '3.8'
  services:
    text-eval-api:
